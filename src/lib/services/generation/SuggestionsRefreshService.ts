...
function log(...args: unknown[]) { console.log('[SuggestionsRefreshService]', ...args); }
...
export interface SuggestionsRefreshDependencies {
  generateSuggestions: (entries: StoryEntry[], activeThreads: StoryBeat[], lorebookEntries: Entry[], promptContext: PromptContext) => Promise<SuggestionsResult>;
...
export class SuggestionsRefreshService {
  ...
  async refresh(input: SuggestionsRefreshInput): Promise<SuggestionsRefreshResult> {
    const { entries, pendingQuests, storyMode, pov, tense, protagonistName, genre, settingDescription, tone, themes, lastLorebookRetrieval, translationSettings } = input;
    ...
    log('Suggestions refreshed:', finalSuggestions.length, 'with', activeLorebookEntries.length, 'active lorebook entries');
