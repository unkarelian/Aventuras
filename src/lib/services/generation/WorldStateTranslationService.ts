...
export interface ClassificationNewEntities {
  newCharacters: Array<{ name: string; description?: string; relationship?: string; traits?: string[]; visualDescriptors?: Record<string, string> }>;
...
export class WorldStateTranslationService {
  ...
  async translateEntities(input: WorldStateTranslationInput, callbacks: WorldStateTranslationCallbacks): Promise<WorldStateTranslationResult> {
    ...
    for (const char of classificationResult.newCharacters) {
      ...
      if (!db) continue;
      items.push({ id: `${db.id}:name`, text: char.name, type: 'name', entityType: 'character', field: 'translatedName' });
      if (char.description) items.push({ id: `${db.id}:desc`, text: char.description, type: 'description', entityType: 'character', field: 'translatedDescription' });
      if (char.relationship) items.push({ id: `${db.id}:rel`, text: char.relationship, type: 'description', entityType: 'character', field: 'translatedRelationship' });
      if (char.traits?.length) items.push({ id: `${db.id}:traits`, text: char.traits.join(', '), type: 'description', entityType: 'character', field: 'translatedTraits', isArray: true });
      if (hasDescriptors(char.visualDescriptors)) items.push({ id: `${db.id}:visual`, text: descriptorsToString(char.visualDescriptors), type: 'description', entityType: 'character', field: 'translatedVisualDescriptors' });
    }
    ...
    for (const loc of classificationResult.newLocations) {
      ...
      if (!db) continue;
      items.push({ id: `${db.id}:name`, text: loc.name, type: 'name', entityType: 'location', field: 'translatedName' });
      if (loc.description) items.push({ id: `${db.id}:desc`, text: loc.description, type: 'description', entityType: 'location', field: 'translatedDescription' });
    }
    ...
    for (const item of classificationResult.newItems) {
      ...
      if (!db) continue;
      items.push({ id: `${db.id}:name`, text: item.name, type: 'name', entityType: 'item', field: 'translatedName' });
      if (item.description) items.push({ id: `${db.id}:desc`, text: item.description, type: 'description', entityType: 'item', field: 'translatedDescription' });
    }
    ...
    for (const beat of classificationResult.newStoryBeats) {
      ...
      if (!db) continue;
      items.push({ id: `${db.id}:title`, text: beat.title, type: 'title', entityType: 'storyBeat', field: 'translatedTitle' });
      if (beat.description) items.push({ id: `${db.id}:desc`, text: beat.description, type: 'description', entityType: 'storyBeat', field: 'translatedDescription' });
    }

    if (items.length === 0) return { translatedCount: 0 };

    log('Translating world state elements', { count: items.length, targetLanguage });
    const translated = await this.deps.translateUIElements(items.map(({ id, text, type }) => ({ id, text, type })), targetLanguage);
    ...
    for (const t of translated) {
      ...
      const value = orig.isArray ? t.text.split(',').map((s) => s.trim()).filter(Boolean) : t.text;
      const data: Record<string, string | string[] | null> = { [orig.field]: value, translationLanguage: targetLanguage };

      if (orig.entityType === 'character') await callbacks.updateCharacter(entityId, data);
      else if (orig.entityType === 'location') await callbacks.updateLocation(entityId, data as Record<string, string | null>);
      else if (orig.entityType === 'item') await callbacks.updateItem(entityId, data as Record<string, string | null>);
      else if (orig.entityType === 'storyBeat') await callbacks.updateStoryBeat(entityId, data as Record<string, string | null>);
    }
