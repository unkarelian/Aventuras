...
class PromptExportService {
  ...
  buildExportData(): PromptExportData {
    const exportedPresets: ExportedGenerationPreset[] = settings.generationPresets.map(
      (preset) => ({
        id: preset.id,
        name: preset.name,
  ...
  parseImportFile(content: string): ParsedPromptImport {
    ...
    if (this.compareVersions(version, EXPORT_VERSION) > 0) {
      warnings.push(
        `File was created with a newer version (${version}). Some features may not import correctly.`,
      )
    }
