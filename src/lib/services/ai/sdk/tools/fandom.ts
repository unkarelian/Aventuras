...
import { tool } from 'ai';
import { z } from 'zod';
import { FandomService, type FandomSearchResult, type FandomArticleInfo, type FandomSectionContent } from '../../../fandom';
...
export function createFandomTools(context: FandomToolContext) {
  ...
  return {
     ...
     */
    search_fandom: tool({
      description: 'Search for articles on a Fandom wiki (e.g., "harrypotter", "starwars", "lotr"). Returns article titles and snippets.',
      ...
      execute: async ({ wiki, query, limit }: { wiki: string; query: string; limit?: number }): Promise<{ success: boolean; results?: FandomSearchResult[]; error?: string }> => {
        try {
    ...
    get_fandom_article_info: tool({
      description: 'Get the structure of a Fandom wiki article including its sections and categories. Use this to understand what content is available before fetching specific sections.',
      inputSchema: z.object({
        wiki: z.string().describe('The wiki subdomain'),
        title: z.string().describe('Exact article title (from search results)'),
      }),
      execute: async ({ wiki, title }: { wiki: string; title: string }): Promise<{ success: boolean; info?: FandomArticleInfo; error?: string }> => {
        try {
    ...
    fetch_fandom_section: tool({
      description: 'Fetch a specific section of a Fandom wiki article by section index. Use "0" for the introduction/lead section. Get section indices from get_fandom_article_info.',
      inputSchema: z.object({
        wiki: z.string().describe('The wiki subdomain'),
        title: z.string().describe('Exact article title'),
        sectionIndex: z.string().describe('Section index (use "0" for intro, or get indices from get_fandom_article_info)'),
      }),
      execute: async ({ wiki, title, sectionIndex }: { wiki: string; title: string; sectionIndex: string }): Promise<{ success: boolean; section?: FandomSectionContent; error?: string }> => {
        try {
