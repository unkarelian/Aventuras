<script lang="ts">
  ...
  import type { QuickStartSeed } from "$lib/services/templates";

  interface Props {
    onClose: () => void;
    /** Optional seed to pre-populate wizard fields (from Quick Start) */
    initialSeed?: QuickStartSeed | null;
    /** Optional pre-selected mode (from Quick Start) */
    initialMode?: StoryMode;
    /** Optional pre-selected POV (from Quick Start) */
    initialPov?: POV;
  }

  let { onClose, initialSeed = null, initialMode, initialPov }: Props = $props();

  // Wizard state
  let currentStep = $state(1);
  const totalSteps = 8;
  let seedInitialized = $state(false);
  ...
  // Initialize from seed (Quick Start) - runs once on mount
  $effect(() => {
    if (seedInitialized) return;
    seedInitialized = true;

    // Apply initialMode if provided
    if (initialMode) {
      selectedMode = initialMode;
    }

    // Apply initialPov if provided
    if (initialPov) {
      selectedPOV = initialPov;
    }
    ...
    if (initialSeed) {
      ...
      const genreMap: Record<string, Genre> = {
        ...
        'Mystery': 'mystery',
        'Horror': 'horror',
        'Slice of Life': 'romance', // closest match
        'Custom': 'custom',
      };
      const mappedGenre = genreMap[initialSeed.genre];
      if (mappedGenre) {
        selectedGenre = mappedGenre;
      } else {
        selectedGenre = 'custom';
        customGenre = initialSeed.genre;
      }

      // Pre-fill setting from location description
      const locationDesc = initialSeed.initialState.startingLocation?.description;
      if (locationDesc) {
        settingSeed = locationDesc;
      }

      // Pre-fill protagonist data
      const proto = initialSeed.initialState.protagonist;
      if (proto) {
      ...
      // Set a default story title based on genre (opening will be generated by wizard)
      storyTitle = `My ${initialSeed.genre} Adventure`;

      // Skip to a later step since we have pre-filled data
      // Skip to step 3 (Genre) since mode is set and we can let them confirm genre
      currentStep = 3;
    }
  });
